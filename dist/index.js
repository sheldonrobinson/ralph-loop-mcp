...
const getStateDir = (sessionId = "default") => {
    const homeDir = process.env.HOME || process.env.USERPROFILE || ".";
    return resolve(homeDir, ".goose", "ralph", sessionId);
};
// State file paths
const getStateFile = (sessionId, fileName) => {
...
class RalphState {
    ...
    validateCrossModelReview() {
        const config = this.getConfig();
        if (!config) {
            return { valid: true }; // Not configured yet
        }
        if (config.crossModelReviewEnforced &&
    ...
    getTask() {
        return this.readJson("task.json");
    }
    // Work management
    setWork(work, summary, iteration) {
        const workData = {
            work,
            summary,
    ...
    getWork() {
        return this.readJson("work.json");
    }
    // Review management
    setReview(decision, feedback, iteration) {
        const reviewData = {
            decision,
            feedback,
    ...
    getReview() {
        return this.readJson("review.json");
    }
    getReviewResult() {
        const result = this.readJson("review-result.txt");
        return result?.decision || null;
    }
    getFeedback() {
        const feedback = this.readJson("review-feedback.txt");
        return feedback?.feedback;
    }
    // Status management
    getStatus(maxIterations = 10) {
    ...
    try {
        switch (name) {
            case "ralph_loop_initialize": {
                ...
                const workerProvider = args?.workerProvider;
                const reviewerModel = args?.reviewerModel;
                const reviewerProvider = args?.reviewerProvider;
                const crossModelReviewEnforced = args?.crossModelReviewEnforced ?? true;
                if (!task) {
